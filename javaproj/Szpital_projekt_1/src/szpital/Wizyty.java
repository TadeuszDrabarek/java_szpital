...
public class Wizyty {
	...
	public void print_wizyty_um(){
		System.out.println(String.format("%-12s|%-12s|%-30s|%-12s|%-30s|%-20s","Data","Czas","Pacjent","PESEL","Lekarz","SpecjalnoϾ"));
		System.out.println("-------------------------------------------------------------------------------------------------------------------");
		try {
			while (db.getRs().next()){
				System.out.println(String.format("%-12s|%-12s|%-30s|%-12s|%-30s|%-20s"
	...
	public void wizyty_wzystkie_um(){
		String sql="select data, czas, imie, nazwisko, pesel, lekarz, nazwaspecjalnosci from v_wizyty_umowione  order by data,czas,nazwisko,imie,idpacjenta;";
		//System.out.println(sql);
		if (db.select(sql)) print_wizyty_um();
		
	}
	
	public void wizyty_lekarza_um(int idl){
		String sql=String.format("select data, czas, imie, nazwisko, pesel, lekarz, nazwaspecjalnosci "
				+ " from v_wizyty_umowione  "
				+ " where idlekarza=%d"
				+ " order by data,czas,nazwisko,imie,idpacjenta;",idl);
		//System.out.println(sql);
		if (db.select(sql)) print_wizyty_um();
		
	}
	
	public void wizyty_pacjenta_um(int idp){
		String sql=String.format("select data, czas, imie, nazwisko, pesel, lekarz, nazwaspecjalnosci "
				+ " from v_wizyty_umowione  "
				+ " where idpacjenta=%d"
				+ " order by data,czas,nazwisko,imie,idpacjenta;",idp);
		//System.out.println(sql);
		if (db.select(sql)) print_wizyty_um();
