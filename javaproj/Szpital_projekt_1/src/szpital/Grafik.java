...
public class Grafik {
	...
	public void print_wizyty(){
		System.out.println(String.format("%-30s|%-20s|%-12s|%-7s|%15s","Lekarz","Specjalnoœæ","Data","Godzina","identyfikator wizyty"));
		System.out.println("-----------------------------------------------------------------------------------------------");
		try {
			while (db.getRs().next()){
	...
	public void umowienie_wizyty(){
		...
		if (idp<0) {
			...
			do {		
				System.out.println("Podaj identyfikator wizyty, któr¹ chcesz umówiæ :");
				uid=rl.nextLine();
				if (uid.equals("")) {
					System.out.println("Przerwanie!");
					break;
				...
				if (W.sprawdz(uid)){
					...
					System.out.println("Nie mo¿na zarezerwowaæ, ten termin jest ju¿ zajêty");
					System.out.println("Wybierz inny termin lub wciœnij ENTER aby zakoñczyæ...");
				}
			} while (true);
		};
